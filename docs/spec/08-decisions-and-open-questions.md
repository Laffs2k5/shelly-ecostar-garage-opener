...
| Q-13 | **Bench-test at least one real reed switch** (mrsmart **MR-00326**, closed-loop NC) into an i4 input — confirm it operates + registers (CLOSED/OPEN) as planned | Much easier on the bench than at the garage; do before install. Rig used opto reed-*sims*; verify a real reed end-to-end (magnet near/away → SW1/SW2). **Polarity confirmed 2026-06-08 (multimeter): magnet present → contact CLOSED** (closed-loop) → matches D-16, `invert:false` correct. Spec: metal case ~25×48 mm, 2-wire, ~10 W max, free-air gap **~5 cm** (shrinks on steel). i4 bench wiring: reed straight between **SW1** and **−**, no opto/resistor (dry contact). *Wave-magnet test deferred to 2026-06-09.* |
| Q-17 | **Publish device-health flags to MQTT for monitoring** (FUTURE — don't explore yet) — surface things like `Script.GetStatus` `running:false` + `errors`/`error_msg` (e.g. the syntax-error crash found 2026-06-08) so a dead script is visible remotely, not just by polling RPC. | Nuance: a *crashed* script can't publish its own death — the mechanism must be **external** to the script (firmware `status_ntf` on the Script component, a cross-device check, or treat the existing **stale retained heartbeat + `mon/alive` gap** as the implicit "something's wrong" signal the monitor spec already leans on). Scope later |
| Q-16 | **Motion-timing & pulse safety** (DEFERRED, [spec 14](14-motion-timing-and-pulse-safety.md)) — the ~5 cm reed window exposes two bugs: (1) `derive()` reports the *old* end-state while *departing* it (reed checked before motor dir); (2) a pulse to an "almost-closed" door is read by the EcoStar as STOP, halting it instead of opening. | **Engineer for it, don't iterate on the real door.** Plan: write failing tests first (incl. **time as a factor** — command landing mid-transition), then fix monitor `derive()` (motor-dir tiebreaker) + controller at-rest guard (`!moving && settledMs >= REST_GUARD`), then eval regression. Data already in the pushed picture (`inputs.op/cl`, `since`/`ts`). Depends on Q-02 (motor opto active *throughout* travel) |
