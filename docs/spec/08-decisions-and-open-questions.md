...
| D-19 | Controller command→pulse: `toggle`=always 1 pulse; directional commands suppress when already there/going, 1 pulse from the matching end/stop, 2 pulses (stop+reverse) when moving the wrong way, and **1 best-effort pulse when door state is UNKNOWN**; **`stop`=safety halt: 1 pulse iff confirmed moving (OPENING/CLOSING), else no-op (never starts a stopped/UNKNOWN door); bypasses the pulse-lockout** | Mirrors the physical button; UNKNOWN (i4 down/boot) still lets remote control work. `stop` added 2026-06-11 (safety + app morphing-button). Implemented in `controller.js`, verified on hardware (spec 14) |
