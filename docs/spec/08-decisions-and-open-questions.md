...
| Q-15 | **AP client isolation** — the phone can't reach the i4/S1 directly (reaches the broker + coffee plug fine), so HTTP-direct (phone→Shelly) is unreliable in this house; the **broker (mTLS) path is primary** | Confirmed on-phone 2026-06-08 (HW-VALIDATION). Network-admin question: put phone+Shellys on a non-isolated segment, or accept broker-path-primary (the roaming design handles it either way) |
