...
| Q-17 | **Publish device-health flags to MQTT for monitoring** (FUTURE — don't explore yet) — surface things like `Script.GetStatus` `running:false` + `errors`/`error_msg` (e.g. the syntax-error crash found 2026-06-08) so a dead script is visible remotely, not just by polling RPC. | Nuance: a *crashed* script can't publish its own death — the mechanism must be **external** to the script (firmware `status_ntf` on the Script component, a cross-device check, or treat the existing **stale retained heartbeat + `mon/alive` gap** as the implicit "something's wrong" signal the monitor spec already leans on). Scope later. **DECIDED 2026-06-10:** add **`rssi`** (WiFi signal) to **OUR retained heartbeat** `devices/<id>/heartbeat` (app-status payload, spec 03) on **BOTH i4 and S1** — *not* to `mon/<id>/alive` (monitor-spec-bound, D-05, leave untouched). Do it in the FW pass; first use is the unattended garage **RSSI-over-time survey** (lowest seen −85 dBm; ESP32 drops ~−90) + ongoing link-quality visibility. Doc-only for now. |
